...
use Carbon\Carbon;
use Barryvdh\DomPDF\Facade\Pdf; // optional, for PDF generation
use App\Services\EmailService;   // <-- use your EmailService
...
class PatientController extends Controller
{
    ...
    public function create()
    {
        ...
        $patient->patient_code = $this->predictNextPatientCode(); // Predict next code for display only
        return view('patients.create', compact('patient'));
    }

    // Store new patient and email next of kin (no PDF attachment)
    public function store(Request $request, EmailService $emailService)
    {
        ...
        do {
            try {
                DB::transaction(function () use (&$validated, &$createdPatient, &$createdInvoice, &$savedPdfFilename, &$pdfBytesLocal) {
                    $validated['patient_code'] = $this->generateNextPatientCode();
                    $createdPatient = PatientDetail::create($validated);

                    // Determine age group
                    $age = $createdPatient->dob ? Carbon::parse($createdPatient->dob)->age : null;
                    $ageGroup = ($age !== null && $age < 18) ? 'child' : 'adult';

                    $fee = ConsultationFee::where('age_group', $ageGroup)->orderByDesc('id')->first() ??
                        ConsultationFee::orderByDesc('id')->first();
                    ...
                    if ($fee) {
                        ...
                        if (class_exists(\Barryvdh\DomPDF\Facade\Pdf::class)) {
                            ...
                            $pdfBytesLocal = $pdf->output();
                            $filename = 'invoice_' . $createdInvoice->invoice_number . '.pdf';
                            $path = 'invoices/' . $filename;
                            Storage::disk('public')->put($path, $pdfBytesLocal);
                ...
                $flashMessage = 'Patient registered successfully.' . (isset($createdInvoice) ? ' Invoice created.' : '');
                ...
                if (!empty($createdPatient->next_of_kin_email) && $createdInvoice) {
                    $subject = 'Patient Registration and Consultation Invoice';
                    $body = $this->buildNextOfKinEmailBody($createdPatient, $createdInvoice);

                    if (method_exists(DB::class, 'afterCommit')) {
                        DB::afterCommit(function () use ($emailService, $createdPatient, $subject, $body) {
                            $resp = $emailService->sendEmailWithAttachment(
                                $createdPatient->next_of_kin_email,
                                $subject,
                                $body,
                                []
                            );
                            ...
                            if (!($resp['success'] ?? false)) {
                                Log::error('Next of kin email send failed', ['error' => $resp['error'] ?? 'unknown']);
                            }
                        });
                    } else {
                        $resp = $emailService->sendEmailWithAttachment(
    ...
    // Build plain-text email body for next of kin
    protected function buildNextOfKinEmailBody(PatientDetail $patient, Invoice $invoice): string
    {
        $lines = [
            ...
            'Email: ' . ($patient->email ?? ''),
            'Address: ' . ($patient->residential_address ?? ''),
            '',
            'Consultation Invoice:',
            '---------------------',
            'Invoice #: ' . $invoice->invoice_number,
            'Issued: ' . ($invoice->issue_date ? $invoice->issue_date->format('Y-m-d') : ''),
